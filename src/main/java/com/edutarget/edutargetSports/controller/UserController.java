...
public class UserController {
    ...
    public ResponseEntity<UserResponse> registerUser(@Valid @RequestBody UserRegistrationRequest req,
        ...
        AppUser creator = appUserRepository.findByUniqueId(loggedInUser)
                .orElseThrow(() -> new ResourceNotFoundException("Logged in user not found"));
        if(!creator.isActive()){
            throw new UserInactiveException("User is not active");
        }

        return ResponseEntity.status(HttpStatus.CREATED).body(userService.registerUser(req, creator));
    }
    ...
    public ResponseEntity<UserResponse> updateUser(@PathVariable String uniqueId,
        ...
        AppUser creator = appUserRepository.findByUniqueId(loggedInUserId)
                .orElseThrow(() -> new ResourceNotFoundException("Logged in user not found"));
        if(!creator.isActive()){
            throw new UserInactiveException("User is not active");
        }

        return ResponseEntity.ok(userService.updateUser(uniqueId, req, creator));
    }
    ...
    public ResponseEntity<UserResponse> changeStatus(@PathVariable String uniqueId,
        ...
        return ResponseEntity.ok(userService.changeUserStatus(uniqueId, active, creator));
    }
}
